...
class ProfileController extends ApiController
{
    ...
    public function following($identifier = null)
    {
        ...
        array_walk($output['member_follows'], function (&$item) {
            if ($item->following_type === 'EO') {
                ...
                if ($user) {
                    $item->details = ['name' => $user->name, 'image' => $user->profile_picture ? base_url('uploads/profile/' . $user->profile_picture) : null];
                } else {
                    $item->details = null;
                }
            }
    ...
    public function activitySummary($identifier = null)
    {
        ...
        if (!$isOwner) {
            $reviewsQuery->where('is_public', 1);
        }
        $reviews_posted_count = $show_ratings ? $reviewsQuery->countAllResults() : 0;
        ...
        foreach ($tickets as $ticket) {
            $activities[] = [
                ...
                'data' => [
                    ...
                    'thumbnail' => $ticket->thumbnail_url ? 'event/' . $ticket->thumbnail_url : null,
                ],
            ...
            foreach ($ratings as $rating) {
                ...
                if ($rating->target_type === 'EVENT') {
                    ...
                    $target_details = $event ? [
                        'title' => $event->title,
                        'thumbnail' => $event->thumbnail_url ? 'event/' . $event->thumbnail_url : null,
                    ] : null;
