/**
 * Facility Verification Detail Modal
 * Modal for reviewing and approving/rejecting facility verification requests
 *
...
const FacilityDetailModal: React.FC<FacilityDetailModalProps> = ({ show, onHide, data, onApprove, onReject }) => {
    if (!data) return null;
    ...
    return (
        <Modal show={show} onHide={onHide} size="lg" centered scrollable>
            <Modal.Header closeButton className="bg-light">
                <Modal.Title>
                    <i className="bx bx-building me-2 text-primary" />
                    Facility Verification Review
                </Modal.Title>
            </Modal.Header>
            ...
            <Modal.Body className="p-0">
                <div className="p-4">
                    ...
                    {/* Header — Facility branding + status */}
                    <div className="d-flex align-items-center mb-4 pb-3 border-bottom">
                        <div className="avatar avatar-xl me-3">
                            {data.logo_path ? (
                                <OneGalery attachments={`/facilities_organizer/${data.logo_path}`} />
                            ) : (
                                <div
                                    className="bg-label-secondary rounded d-flex align-items-center justify-content-center"
                                    style={{ width: '72px', height: '72px' }}
                                >
                                    <i className="bx bx-building fs-1" />
                                </div>
                            )}
                        </div>
                        <div className="flex-grow-1">
                            <h4 className="mb-1">{data.facility_name}</h4>
                ...
                {(!data.verification_status || data.verification_status === 'Pending') && (
                    <div className="d-flex gap-2">
                        ...
                        <Button variant="success" onClick={() => onApprove(data.id)}>
                            <i className="bx bx-check me-1" />Approve Facility
                        </Button>
                    </div>
                )}
            </Modal.Footer>
